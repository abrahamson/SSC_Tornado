c -------------------------------------------------------------------
      ...
      subroutine read_out5 ( nFlt, haz, nAtten, nFtype, iPer, nProb)
...
c          write (*,'( 20i5)') iFlt, nFlt, jProb, nProb,  nAtten(iFlt), nWidth(iFlt), nFtype(iFlt), 
...
c        if (nAtten(iFlt) .gt. MAX_ATTEN) then
c          write (*,*) 'MAX_ATTEN needs to be increased to ', nAtten(iFlt)
c          stop 99
c        endif
         if (nWidth(iFlt) .gt. MAX_WIDTH) then
           write (*,*) 'MAX_WIDTH needs to be increased to ', nWidth(iFlt)
